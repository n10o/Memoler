...
public abstract class BsMemberBhv extends AbstractBehaviorWritable {
     ...
     */
    public void loadInfoList(Member member, ConditionBeanSetupper<InfoCB> conditionBeanSetupper) {
        xassLRArg(member, conditionBeanSetupper);
        loadInfoList(xnewLRLs(member), conditionBeanSetupper);
    }
    /**
     * Load referrer of infoList with the set-upper for condition-bean of referrer. <br />
     * INFO by MEMBER_ID, named 'infoList'.
     * <pre>
     * memberBhv.<span style="color: #FD4747">loadInfoList</span>(memberList, new ConditionBeanSetupper&lt;InfoCB&gt;() {
     *     public void setup(InfoCB cb) {
     *         cb.setupSelect...();
     *         cb.query().setFoo...(value);
     ...
     * });
     * for (Member member : memberList) {
     *     ... = member.<span style="color: #FD4747">getInfoList()</span>;
     * }
     * </pre>
     ...
     * @param conditionBeanSetupper The instance of referrer condition-bean set-upper for registering referrer condition. (NotNull)
     */
    public void loadInfoList(List<Member> memberList, ConditionBeanSetupper<InfoCB> conditionBeanSetupper) {
        xassLRArg(memberList, conditionBeanSetupper);
        loadInfoList(memberList, new LoadReferrerOption<InfoCB, Info>().xinit(conditionBeanSetupper));
    }
    /**
     * {Refer to overload method that has an argument of the list of entity.}
     * @param member The entity of member. (NotNull)
     ...
     */
    public void loadInfoList(Member member, LoadReferrerOption<InfoCB, Info> loadReferrerOption) {
        xassLRArg(member, loadReferrerOption);
        loadInfoList(xnewLRLs(member), loadReferrerOption);
    }
    /**
     ...
     * @param loadReferrerOption The option of load-referrer. (NotNull)
     */
    public void loadInfoList(List<Member> memberList, LoadReferrerOption<InfoCB, Info> loadReferrerOption) {
        xassLRArg(memberList, loadReferrerOption);
        if (memberList.isEmpty()) { return; }
        final InfoBhv referrerBhv = xgetBSFLR().select(InfoBhv.class);
        helpLoadReferrerInternally(memberList, loadReferrerOption, new InternalLoadReferrerCallback<Member, Long, InfoCB, Info>() {
            public Long getPKVal(Member e)
            { return e.getMemberId(); }
            public void setRfLs(Member e, List<Info> ls)
            { e.setInfoList(ls); }
            public InfoCB newMyCB() { return referrerBhv.newMyConditionBean(); }
            public void qyFKIn(InfoCB cb, List<Long> ls)
            { cb.query().setMemberId_InScope(ls); }
            public void qyOdFKAsc(InfoCB cb) { cb.query().addOrderBy_MemberId_Asc(); }
            public void spFKCol(InfoCB cb) { cb.specify().columnMemberId(); }
            public List<Info> selRfLs(InfoCB cb) { return referrerBhv.selectList(cb); }
            public Long getFKVal(Info e) { return e.getMemberId(); }
            public void setlcEt(Info re, Member le)
            { re.setMember(le); }
            public String getRfPrNm() { return "infoList"; }
        });
    }
